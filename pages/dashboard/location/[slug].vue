...
async function confirmDelete() {
  try {
    ...
    await $fetch(`/api/locations/${route.params.slug}`, { method:
      "DELETE" });
...
<template>
  <div class="page-content-top">
          ...
          >
            <li>
              <NuxtLink @click="openDialog">
                <Icon name="tabler:trash" size="20" /> Delete
              </NuxtLink>
            </li>
            <li>
      ...
      >
        <template #top>
          <p class="text-sm italic text-gray-500">
            <span v-if="log.startedAt !== log.endedAt" class="">
              {{ formatDate(log.startedAt) }} / {{ formatDate(log.endedAt) }}
            </span>
            <span v-else class="">{{ formatDate(log.startedAt) }}</span>
          </p>
        </template>
      </LocationCard>
    </div>
    ...
    <AppDialog
      :is-open="isOpen"
      title="Are you sure?"
      description="Deleting this location will also delete all of it associated logs. This cannot be undone."
